...
func create_term():
   if not(GlobalDatabase.exists("term", edit_subject.text, edit_topic.text, edit_term.text)):
     var tmp = Term.new()
     ...
     tmp.creation_day = Date.new()
     tmp.creation_day.set_today()
     tmp.actuality_day_count = GlobalDatabase.term_returning_cycles[0]
     GlobalDatabase.add_term(tmp)
     log_text("Card/Term '" + edit_term.text.left(12) + "' created.")
     edit_term.text = ""
     edit_definition.text = ""
   else:
     GlobalFunctions.show_warning(self, "", "The card already exists in the given categories.")
   pass
...
func focus_edit_term():
   edit_term.grab_focus()
   pass
